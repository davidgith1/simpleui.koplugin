...
-- recently read. Same visual layout as Recent Books (module_book_rows.lua),
-- but built independently rather than through RowRenderer.makeModule: that
...
-- gate below); until the user opens Rakuyomi once in the session, the row
-- stays empty rather than forcing a cold start.
...
function M.build(w, ctx)
    ...
    local border_color = (ok_ss and SUIStyle2 and SUIStyle2.getThemeColor("separator")) or Blitbuffer.gray(0.72)
    local bg_color = solid_bg and ((ok_ss and SUIStyle2 and SUIStyle2.getThemeColor("bg")) or Blitbuffer.COLOR_WHITE) or nil
...
-- Deliberately NOT RowRenderer.getHeight(): that formula always reserves
